...
int currentBufSize = DEFAULT_TABLE_BUFFER_SIZE + 1;
...
extern RC initRecordManager (void *mgmtData){
    ...
    if ( init_record_manager == 0 ){
        ...
        if (mgmtData != NULL) {
            ...
            currentBufSize = DEFAULT_TABLE_BUFFER_SIZE + 1; // +1 for bitmap always fixed page
        }
...
extern RC openTable (RM_TableData *rel, char *name){
    ...
    return_code = initBufferPool(bManager, file_name, currentBufSize, rep_stratergy, NULL);
    if( return_code != 0)
        return return_code;
